...
- **Virtualisierung & Docker-Grundlagen** ([modules/containers.html](modules/containers.html)) - Typ-1- vs. Typ-2-Hypervisor, virtuelle Maschinen vs. Container, Docker-Kernbegriffe (Image/Container/Dockerfile/Registry), inkl. Reihenfolge-Puzzle "vom Code zum laufenden Container" sowie einer Praxisaufgabe: eigenen Immich-Fotoserver per Docker Compose auf einer VM aufsetzen, Testfotos hochladen und den Zugriff von einer zweiten VM im selben virtuellen Netz verifizieren.
...
- **API-Grundlagen** ([modules/api-basics.html](modules/api-basics.html)) - Was eine API ist, REST-Prinzipien, HTTP-Methoden (CRUD), Statuscodes, Aufbau einer Anfrage (Pfad-/Query-Parameter, Body, Header) - inkl. simulierter REST-API (selbstgebauter Interpreter) über ein realistisches Beispiel mit zwei verknüpften Ressourcen (Abteilungen/Mitarbeiter): verschachtelte Routen, Filtern/Suchen/Sortieren/Paginieren per Query-Parameter, simulierter Bearer-Token-Schutz auf schreibenden Endpunkten, live sichtbarer Backend-Zustand, Request-Sandbox und 12 schrittweisen Aufgaben (inkl. 401/400/404-Statuscode-Verhalten).
...
- **Netzwerkpakete, TCP/UDP & OSI-Modell** ([modules/network-packets.html](modules/network-packets.html)) - Kapselung/Paketaufbau (inkl. detailliertem IP-Header-Bitfeld), TTL, interaktive TCP-Drei-Wege-Handschlag-Animation mit sichtbarem Paketinhalt (Flags/Seq/Ack, Abspielen/Schritt-für-Schritt), TCP-vs-UDP-Analogie-Grafik, OSI- vs. TCP/IP-Modell, inkl. OSI-Reihenfolge-Puzzle und Port-↔-Dienst-Zuordnungsspiel.
- **Netzwerkgeräte & Routing** ([modules/network-devices.html](modules/network-devices.html)) - Hub/Switch/Router im Vergleich, interaktive ARP-Auflösung-Animation mit sichtbarem Paketinhalt (Broadcast-Request + Unicast-Reply), VLANs (Broadcast-Domänen-Trennung, Access- vs. Trunk-Port, 802.1Q-Tagging, Inter-VLAN-Routing), plus generierte Routing-Tabellen-Aufgaben (Longest Prefix Match).
...
- **DNS-Auflösung & DHCP-Prozess** ([modules/dns-dhcp-basics.html](modules/dns-dhcp-basics.html)) - interaktive Animationen mit sichtbarem Paketinhalt (Abspielen/Schritt-für-Schritt) des DHCP-DORA-Prozesses (inkl. Broadcast an mehrere Server) und der rekursiven DNS-Auflösung über Root-/TLD-/autoritativen Server, plus Referenztabelle aller DNS-Record-Typen, inkl. DORA-Reihenfolge-Puzzle und Record-Typ-↔-Zweck-Zuordnungsspiel. Zusätzlich **Praxislabor Teil 1**: echte DHCP/DNS-Testumgebung mit VMs aufbauen (Downloadlinks zu VirtualBox/Hyper-V, Windows-Server- und Windows-11-Evaluation-ISOs, Schritt-für-Schritt-Anleitung inkl. localStorage-Checkliste) - wird im Praxislabor Teil 2 (Active Directory) direkt weiterverwendet.
- **DHCP/DNS-Troubleshooting** ([modules/dhcp-dns.html](modules/dhcp-dns.html)) - zehn Helpdesk-Tickets mit simulierten Tool-Ausgaben.
- **VPN-Grundlagen** ([modules/vpn-basics.html](modules/vpn-basics.html)) - Site-to-Site vs. Client-to-Site, IPSec vs. SSL-VPN (inkl. Vergleichsdiagramm), Split- vs. Full-Tunneling, interaktive Tunnelaufbau-Animation (IKE Phase 1/2, Datenkapselung mit sichtbarem äusserem/innerem Paket), sowie einer Praxisaufgabe: echten WireGuard-Tunnel zwischen SRV01 und CL01 aufbauen und mit Wireshark verifizieren, dass der Tunnel-Verkehr verschlüsselt ist.
...
- **Intune / Entra ID / Hybrid** ([modules/intune-entra.html](modules/intune-entra.html)) - Join-Typen, Conditional Access (Kontrast zum Firewall-Modul: kumulative statt erste-Regel-Logik), Gerätemanagement-Quiz, Tickets. Am Modulende zwei weitere Praxislabor-Teile: **Teil 6** eigener Cloud-Tenant (CDX oder Microsoft 365 Developer Program als Fallback), ein neues Gerät bei Intune registrieren, Konfigurationsprofil + Win32-App (7-Zip) + Unternehmensportal; **Teil 7 (Abschluss des gesamten Laborbogens)** SRV01 per Microsoft Entra Connect hybrid mit dem Tenant verbinden - bewusst gefiltert auf nur Benutzer/Gruppen, damit die Geräteverwaltung vollständig bei Intune bleibt (kein Hybrid Azure AD Join).
